...
% Diagrams are pinned with [H] so each sits under the heading that introduces it.
...
This document is the deliverable for step 3.5: it defines the requirements (3.1),
maps the components (3.2), justifies the technology stack (3.3) and presents the
...
NFR-12 & Portability     & Frontend is a static bundle for any CDN & static-host deploy test \\
...
Routing         & HashRouter    & NFR-12       & BrowserRouter --- needs server rewrites a static host cannot do \\
...
SSE connection; behind it the feed can be substituted without touching any
component above.}
\end{figure}
